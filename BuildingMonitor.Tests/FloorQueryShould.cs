...
namespace BuildingMonitor.Tests
{
    public class FloorQueryShould : TestKit
    {
        ...
        public void ReturnTemperatures()
        {
            ...
            var floorQuery = Sys.ActorOf(FloorQuery.Props(actorToSensorId: new Dictionary<IActorRef, string>
                {
            ...
            var temperatureReading1 =
                Assert.IsAssignableFrom<TemperatureAvailable>(response.TemperatureReadings["sensor1"]);
            Assert.Equal(23.9, temperatureReading1.Temperature);

            var temperatureReading2 =
                Assert.IsAssignableFrom<TemperatureAvailable>(response.TemperatureReadings["sensor1"]);
